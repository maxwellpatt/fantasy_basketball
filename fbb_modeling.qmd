...
# Ensure that xwell_2024_rating column exists
if (!"xwell_2024_rating" %in% names(final_data)) {
  final_data <- final_data %>%
    mutate(xwell_2024_rating = NA_integer_)
}

# Function to update a player's rating in the xwell_2024_rating column
enter_fantasy_value <- function(player_name, new_ranking) {
  final_data <<- final_data %>%
    mutate(xwell_2024_rating = ifelse(Player == player_name, new_ranking, xwell_2024_rating))

  # Write the updated data to the CSV file
  write_csv(final_data, here::here("Data", "final_data.csv"))
  cat("Player value updated successfully!\n")
}

# Function to describe interest in player (out of 10)
enter_interest <- function(player_name, interest_level) {
  final_data <<- final_data %>%
    mutate(interest = ifelse(Player == player_name, interest_level, interest))

  # Write the updated data to the CSV file
  write_csv(final_data, here::here("Data", "final_data.csv"))
  cat("Player interest updated successfully!\n")
}
...
scatter_plot <- ggplot(final_data, aes(x = variable, y = FPTS)) +
  geom_point(alpha = 0.3) + # Semi-transparent points
  theme_light()

# Box plot for range within each age
box_plot <- ggplot(final_data, aes(x = as.factor(variable), y = FPTS)) +
  geom_boxplot() +
  theme_light()
...
```{find_strongest_correlations <- function(data, target_col) {}
  ...
  correlations <- map_dbl(names(numeric_data), function(col) {
    if (col != target_col) {
      ...
      complete_pairs <- sum(!is.na(numeric_data[[col]]) & !is.na(numeric_data[[target_col]]))
      if (complete_pairs > 0) {
...
weights <- c("FPTS" = 0.5, "Age" = -0.5, "G" = 0.05, "MP" = 0.3, "FG" = 0.15,
             "BLK" = 0.03, "TRB" = 0.07) # Example weights
...
write.csv(final_data_with_ranking, file = "final_data_with_ranking_v3.csv", row.names = FALSE)
